...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      ...
      // Attempt restore from local draft
      try {
        const draft = loadOnboardingProgress(user?.id, 'full');
        if (draft && draft.data) {
          setFormData(prev => ({ ...prev, ...draft.data }));
          if (typeof draft.last_step === 'number') {
            setCurrentStep(Math.max(0, Math.min(draft.last_step, ONBOARDING_STEPS.length - 1)));
          }
  ...
  const saveStepProgressLocal = (stepData) => {
    try {
      const saved = saveOnboardingStep(user?.id, 'full', currentStep, stepData);
  ...
  const handleStepData = async (newData) => {
    ...
    // Save local draft when data changes
    saveStepProgressLocal(updatedFormData);
